...
train_data, val_data, test_data = T.RandomLinkSplit(
    num_val=0.1,
    num_test=0.1,
    edge_types=[('paper', 'to', 'term')],
    rev_edge_types=[('term', 'to', 'paper')]
)(graph)
...
class HGT_LP(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        ...
        self.conv2 = HGTConv(hidden_channels, out_channels, graph.metadata(), heads=4)
...
def test(model):
    ...
    out = model(val_data,
                val_data['paper', 'to', 'term'].edge_label_index).view(-1)
    val_loss = criterion(out, val_data['paper', 'to', 'term'].edge_label)
    # cal metrics
    out = model(test_data,
                test_data['paper', 'to', 'term'].edge_label_index).view(-1).sigmoid()
    model.train()

    auc, f1, ap = get_metrics(out, test_data['paper', 'to', 'term'].edge_label)

    return val_loss, auc, ap


def train():
    save_model_path = root_path + '/models/hgt.pkl'
    model = HGT_LP(-1, 64, 128).to(device)
    ...
    for epoch in tqdm(range(100)):
        optimizer.zero_grad()
        edge_label_index = train_data['paper', 'to', 'term'].edge_label_index
        edge_label = train_data['paper', 'to', 'term'].edge_label
